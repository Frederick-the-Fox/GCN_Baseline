...
def evaluate(embeds, ratio, idx_train, idx_val, idx_test, label, nb_classes, device, dataset, lr, wd
    ...
    for kkk in range(50):
        ...
        for iter_ in range(200):
            ...
            print("logits:{}, train:{}".format(logits.cpu().detach().numpy(), train_lbls.cpu().detach().numpy()))
        ...
        # print(test_lbls.detach().cpu().numpy(), best_proba.detach().cpu().numpy())
...
if __name__ == '__main__':
    ...
    device = 'cpu'
    sc = 3.0
    data = pkl.load(open('/home/hangni/WangYC/pygcn/data/mydata/'+dataset+'/'+dataset+'_new.pkl', "rb"))
    ...
    embeds = np.load(dataset + sys.argv[2] + '.npy')
    ...
    for i in range(len(idx_train)):
        evaluate(torch.tensor(embeds), ratio[i], idx_train[i], idx_val[i], idx_test[i], label, int(sys.argv[3]), device, dataset,0.01, 0)#dblp:4 imdb/freebase:3